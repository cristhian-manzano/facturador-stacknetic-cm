/**
 * End-to-end Supertest assertions for SPEC-0006 — error model & logging
 * wired into apps/api.
 *
 * Coverage:
 *   - Phase 6.1: POST /v1/_diag/echo
 *       * Bad payload → 400 + ProblemDetail with `errors[]` length ≥ 1.
 *       * Good payload → 200 + parsed body echoed (no password leak).
 *   - Phase 6.2: GET /v1/_diag/forced-error
 *       * Each AppError subclass returns its (status, code) pair.
 *       * Body parses through `ProblemDetailSchema`.
 *       * Body never contains stack traces or raw error messages.
 *   - Request id: header echoed when client supplies one, minted (ULID
 *     shape) when absent.
 *   - Redaction: log lines emitted during the request flow MUST NOT contain
 *     any of the project-wide sensitive values. We assert against the
 *     captured stream that none of the sample sensitive substrings appear.
 *
 * The test injects a logger configured with a custom `destination` stream
 * (no `pino-pretty` transport — pure JSON) so we can read every line back
 * and run assertions on it.
 */
import { Writable } from "node:stream";

import request from "supertest";
import { describe, expect, it } from "vitest";

import { ProblemDetailSchema } from "@facturador/contracts/errors";
import { createLogger } from "@facturador/logger";

import { createApp } from "./server.js";

interface CapturedLine {
  level: string;
  service: string;
  env: string;
  pid: number;
  hostname: string;
  time: string;
  msg?: string;
  [key: string]: unknown;
}

function captureSink(): { sink: Writable; lines: () => CapturedLine[] } {
  const buffers: Buffer[] = [];
  const sink = new Writable({
    write(chunk: Buffer, _enc, cb) {
      buffers.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk as unknown as string));
      cb();
    },
  });
  return {
    sink,
    lines: () =>
      Buffer.concat(buffers)
        .toString("utf8")
        .split("\n")
        .filter((line) => line.trim().length > 0)
        .map((line) => JSON.parse(line) as CapturedLine),
  };
}

// Minimal stub Prisma — only the surface area `/health-db` would call.
// We never hit `/health-db` from this suite so it stays unused, but it
// prevents the factory from touching the real DB client at import time.
function stubPrisma() {
  return {
    $queryRaw: () => Promise.resolve(),
    $disconnect: () => Promise.resolve(),
  } as unknown as Parameters<typeof createApp>[0] extends { prisma?: infer P } ? P : never;
}

// Sample sensitive substrings — appearing in any captured log line would
// indicate a redaction regression. Match SPEC-0006 §6.3 do-not-log list.
const SENSITIVE_LITERALS = [
  "supersecret-password-value-9X3",
  "BEGIN PRIVATE KEY",
  "raw-pkcs12-bytes",
  "<factura><signedXml>",
  "sri-clave-acceso-12345",
  "Bearer leaked-bearer-token-42",
  "sessioncookie=leaked-cookie-7",
];

const buildApp = () => {
  const { sink, lines } = captureSink();
  const logger = createLogger({ service: "api", env: "test", destination: sink });
  const app = createApp({ prisma: stubPrisma(), logger, disableOriginCheck: true });
  return { app, lines };
};

describe("apps/api — error model & logging end-to-end", () => {
  describe("X-Request-Id", () => {
    it("echoes the inbound x-request-id header when valid", async () => {
      const { app } = buildApp();
      const id = "01HX8K0PYFA9B7Y1M2N3P4Q5R6";

      const res = await request(app).get("/health").set("x-request-id", id);

      expect(res.status).toBe(200);
      expect(res.headers["x-request-id"]).toBe(id);
    });

    it("mints a fresh ULID when no inbound header is present", async () => {
      const { app } = buildApp();

      const res = await request(app).get("/health");

      expect(res.status).toBe(200);
      const id = res.headers["x-request-id"];
      expect(typeof id).toBe("string");
      // ULID: 26 chars, Crockford base32 (0-9 + A-Z minus I, L, O, U).
      expect(id).toMatch(/^[0-9A-Z]{26}$/i);
    });
  });

  describe("POST /v1/_diag/echo", () => {
    it("returns 200 for a valid payload (and does not echo password)", async () => {
      const { app } = buildApp();

      const res = await request(app)
        .post("/v1/_diag/echo")
        .send({ email: "USER@Example.com", password: "long-enough-password" })
        .set("Content-Type", "application/json");

      expect(res.status).toBe(200);
      expect(res.body).toMatchObject({ ok: true, email: "user@example.com" });
      expect(JSON.stringify(res.body)).not.toContain("long-enough-password");
    });

    it("returns a valid ProblemDetail (400) with errors[] on a bad payload", async () => {
      const { app } = buildApp();

      const res = await request(app)
        .post("/v1/_diag/echo")
        .send({ email: "not-an-email" })
        .set("Content-Type", "application/json");

      expect(res.status).toBe(400);
      const parsed = ProblemDetailSchema.safeParse(res.body);
      expect(parsed.success).toBe(true);
      if (!parsed.success) return;
      expect(parsed.data.code).toBe("validation.failed");
      expect(parsed.data.status).toBe(400);
      expect(parsed.data.errors?.length ?? 0).toBeGreaterThanOrEqual(1);
      // identificador is the offending field's dotted path.
      const idents = parsed.data.errors?.map((m) => m.identificador) ?? [];
      expect(idents).toEqual(expect.arrayContaining(["email"]));
      // instance is the request id we just minted.
      expect(parsed.data.instance).toMatch(/^[0-9A-Z]{26}$/i);
    });
  });

  describe("GET /v1/_diag/forced-error — full subclass matrix", () => {
    const cases = [
      { type: "auth", status: 401, code: "auth.unauthenticated" },
      { type: "forbidden", status: 403, code: "tenant.forbidden" },
      { type: "not_found", status: 404, code: "invoice.not_found" },
      { type: "conflict", status: 409, code: "invoice.duplicate_clave" },
      { type: "rate_limit", status: 429, code: "rate_limited" },
      { type: "upstream", status: 502, code: "sri.network" },
      { type: "business", status: 422, code: "invoice.totals_mismatch" },
      { type: "validation", status: 400, code: "validation.failed" },
      { type: "unknown", status: 500, code: "internal.unexpected" },
    ] as const;

    it.each(cases)(
      "type=$type → $status / $code (ProblemDetail parses)",
      async ({ type, status, code }) => {
        const { app } = buildApp();

        const res = await request(app).get(`/v1/_diag/forced-error?type=${type}`);

        expect(res.status).toBe(status);
        const parsed = ProblemDetailSchema.safeParse(res.body);
        expect(parsed.success).toBe(true);
        if (!parsed.success) return;
        expect(parsed.data.status).toBe(status);
        expect(parsed.data.code).toBe(code);
        expect(parsed.data.type).toBe(`urn:facturador:error:${code}`);
      },
    );

    it("unknown thrown values never leak the original message to the client", async () => {
      const { app } = buildApp();

      const res = await request(app).get("/v1/_diag/forced-error?type=unknown");

      // The server-side throw uses "boom — unexpected" but the client envelope
      // must NEVER carry the raw message body (SPEC-0006 §10 + security
      // policy). The string `internal.unexpected` is the legitimate code, so
      // we assert on the original-message specifics instead.
      const serialised = JSON.stringify(res.body);
      expect(serialised).not.toContain("boom");
      expect(serialised).not.toContain("— unexpected");
      expect(res.body).toMatchObject({
        title: "Internal Server Error",
        status: 500,
        code: "internal.unexpected",
      });
      // No `detail` (toProblemDetail strips raw error messages on the
      // unknown-error path).
      expect((res.body as { detail?: string }).detail).toBeUndefined();
    });
  });

  describe("Redaction in real log output", () => {
    it("never serialises sensitive values into captured log lines", async () => {
      const { app, lines } = buildApp();

      // Drive a happy-path POST that submits every sensitive shape we care
      // about. Pino redaction must scrub the captured stream.
      await request(app)
        .post("/v1/_diag/echo")
        .set("Authorization", "Bearer leaked-bearer-token-42")
        .set("Cookie", "sessioncookie=leaked-cookie-7")
        .send({
          email: "user@example.com",
          password: "supersecret-password-value-9X3",
          // The validator strips these (Zod schema only allows email+password)
          // but if downstream code ever logs the raw body, the redactor must
          // still mask them.
          privateKey: "BEGIN PRIVATE KEY",
          p12: "raw-pkcs12-bytes",
          signedXml: "<factura><signedXml>full</signedXml></factura>",
          claveAcceso: "sri-clave-acceso-12345",
        });

      const captured = lines();
      const serialised = captured.map((l) => JSON.stringify(l)).join("\n");

      for (const literal of SENSITIVE_LITERALS) {
        expect(serialised).not.toContain(literal);
      }
    });

    it("emits a request line with requestId on finish", async () => {
      const { app, lines } = buildApp();
      const id = "01HX8K0PYFA9B7Y1M2N3P4Q5R6";

      await request(app).get("/health").set("x-request-id", id);

      const captured = lines();
      const reqLine = captured.find((l) => l.msg === "request");
      expect(reqLine).toBeDefined();
      if (!reqLine) return;
      expect(reqLine.requestId).toBe(id);
      expect(reqLine.method).toBe("GET");
      expect(reqLine.path).toBe("/health");
      expect(reqLine.status).toBe(200);
    });

    it("emits a request_error line that masks any sensitive payload", async () => {
      const { app, lines } = buildApp();

      // The validator will reject and the handler logs `{ err, problem }`.
      // The err object's path includes the request body; password must not
      // appear in the captured stream.
      await request(app)
        .post("/v1/_diag/echo")
        .send({ email: "x", password: "supersecret-password-value-9X3" });

      const captured = lines();
      const errLine = captured.find((l) => l.msg === "request_error");
      expect(errLine).toBeDefined();
      if (!errLine) return;
      expect(JSON.stringify(errLine)).not.toContain("supersecret-password-value-9X3");
    });
  });
});
